...
// v7.92 — 2026-04-01 — EO-175R PATCH 4:
//         CHANGE D: EO-152 auto-repair DISABLED for ER composite. Empty-URL refs are
//         intentional (approved-sources pipeline) — not broken. Standalone path (PA/GO/SO/PI)
//         does NOT have EO-152 and works correctly. 2×14 refs × 4 retries = 18 min wasted.
//         CHANGE E: Same fix for Activities composite EO-152 block.
// v7.91 — 2026-04-01 — EO-175R PATCH 2 CHANGE B: repairBrokenReferenceUrls gains a circuit
//         breaker — if 3 consecutive API calls return MODEL_OVERLOADED (503/overloaded),
//         the loop stops early and all remaining broken refs are marked verificationStatus:
//         'manual-review', verificationMethod: 'circuit_breaker_stop' instead of retrying
//         indefinitely. Prevents 10+ minute hangs when the model is under heavy load.
//         consecutiveOverloaded resets to 0 on any successful API response.
//         Summary log at end reports groundingFixed / scholarFailed / circuitStopped counts.
// v7.90 — 2026-04-01 — EO-175R: Post-generation safety net in _runFullReferencePipeline STEP 6.
//         After EO-173 URL strip, if options.approvedSources is provided, any ref URL that
//         cannot be matched against an approved source URL or DOI is stripped and marked
//         verificationStatus: 'not_in_approved_sources'. Only refs belonging to the current
//         sectionKey are checked — refs from other sections pass through unchanged.
//         Pipeline calls for PM, risks, and activities_wp in runComposite now pass their
//         local _pmApprovedSources / _rsApprovedSources / _acApprovedSources to the pipeline
//         via options.approvedSources so the safety net is actually triggered.
// v7.89 — 2026-04-01 — EO-174b: Extend approved sources pipeline to activities_wp.
//         'activities' added to APPROVED_SOURCE_SECTIONS in executeGeneration.
//         In executeGeneration activities branch: _approvedSourcesBlock + _execRefsEnabled
//         passed to all 3 generateActivitiesPerWP call sites (regenerate, add-missing-WP,
//         fill-empty-WP paths).
//         In runComposite: BEFORE Step 3 (activities), searchAuthoritativeSources called for
//         'activities' if _refsEnabled130h is true. Result passed as approvedSourcesBlock +
//         referencesEnabledParam to both composite generateActivitiesPerWP call sites.
//         buildSearchQuery (sourceRetrievalService.ts v1.2) now returns short keyword queries
//         (≤100 chars) for activities/risks/projectManagement — no more overlong title+description
//         strings that hurt CrossRef/OpenAlex relevance.
//         Sources accumulated into _approvedSources for BUG29 persist to newData.approvedSources.
// v7.88 — 2026-04-01 — EO-174: Extend approved sources pipeline (CrossRef + OpenAlex) to risks
//         and projectManagement in runComposite. BEFORE Step 1 (PM) and Step 5 (Risks), call
//         searchAuthoritativeSources + formatApprovedSourcesForPrompt and inject the resulting
//         block into the 7th argument of generateSectionContent (approvedSourcesBlock). PM and
//         risks approved sources accumulate into _approvedSources so BUG29 persist block (line
//         ~5798) saves them to newData.approvedSources. Also adds 'risks' and 'projectManagement'
//         to APPROVED_SOURCE_SECTIONS in executeGeneration for standalone regeneration path.
//         _approvedSources in runComposite changed from const→let to allow accumulation.
// v7.87 — 2026-04-01 — EO-173 addendum: Retroactive URL cleanup in _verifyUrlsAfterSave (EO-173B).
//         Before URL verification runs, existing Scholar query + Vertex redirect URLs in savedRefs
//         are stripped synchronously and persisted via setProjectData + storageService.
//         Cleaned refs are excluded from the toVerify batch (no point verifying empty URLs).
// v7.86 — 2026-04-01 — EO-173: Fix 3 root causes of broken/invalid reference URLs.
//         RC1 (Scholar query URL): repairBrokenReferenceUrls Scholar fallback no longer stores
//           scholar.google.com/scholar?q= as ref.url. Marks as {url:'', urlVerified:false,
//           verificationStatus:'not_found'} instead of fake urlVerified=true.
//         RC2 (Vertex AI redirect): repairBrokenReferenceUrls now strips
//           vertexaisearch.cloud.google.com/grounding-api-redirect URLs returned by grounding
//           before accepting them (falls through to not_found rather than storing inaccessible URL).
//         RC3 (post-processing): _runFullReferencePipeline Step 6 strips Scholar query + Vertex
//           redirect URLs from ALL refs in _proposedRefs (cleans up stale data from prior saves).
//           _convertAiRefsToReferences also strips Scholar query URLs (alongside existing EO-112b).
// v7.85 — 2026-04-01 — EO-172: Re-enable referenceRepair with 4-safeguard loop prevention.
//         SAFEGUARD 1: Module-level _repairState {runId, hasRun, attemptedUrls} — persists across renders.
//         SAFEGUARD 2: Reset at start of executeGeneration AND runComposite — one repair pass per run.
//         SAFEGUARD 3: hasRun=true set BEFORE async repair — any concurrent _verifyUrlsAfterSave
//           call (all 3 call sites, all sections PA/GO/SO/PI/AC/PM/RS/ER) is blocked by guard.
//         SAFEGUARD 4: attemptedUrls tracks every URL attempted — prevents retry if re-entered.
//           Save via setProjectData (React state + storageService) — hasRun=true prevents re-run
//           even if setProjectData triggers any downstream verification path.
// v7.84 — 2026-04-01 — EO-170: Fix composite data flow — risks processed content was discarded.
//         ROOT CAUSE: risks schema (EO-159 BUG12) wraps array under "items" key, not "risks".
//         Gemini returns { items: [...], _references: [...] } but Step B only checked .risks →
//         risksContent = { items: [...] } (not array) → Step C: "unexpected format, keeping original"
//         → entire new generation (with [RS-N] markers) discarded, old data kept.
//         FIX: Step B now handles both { risks: [...] } (legacy) and { items: [...] } (schema-correct).
//         Added EO-170 Part D debug logging for both risks and projectManagement data flows.
//         Part B (PM) verified correct — no fix needed (direct object schema, spread merge preserves markers).
//         Part C (activities_wp) verified intentionally disabled (referencesEnabled: false per EO-MASTER E2).
// v7.83 — 2026-03-31 — EO-168c PROBLEM 1: Fix ReferenceError: _postSaveSignal not defined in runComposite.
//         _postSaveSignal is declared in executeGeneration scope and is inaccessible in runComposite.
//         Declare local `const _postSaveSignal: AbortSignal | undefined = undefined;` so that
//         _verifyUrlsAfterSave call sites inside runComposite pass undefined (= no abort) safely.
// v7.82 — 2026-03-31 — EO-168b: Fix ReferenceError: _approvedSources not defined in runComposite.
//         _approvedSources was declared in executeGeneration scope but accessed in the separate
//         runComposite function — declare local empty array so EO-159 BUG29 persist blocks
//         skip gracefully (length=0) instead of crashing the save step.
// v7.81 — 2026-03-31 — EO-168: Disable EO-159 BUG15 repair block in _verifyUrlsAfterSave
//         (was overwriting pipeline references 10s after generation).
//         Add _asyncPostSaveAbortController — kills stale async verify/repair on new generation.
//         EO-152 repair callbacks guarded against double-execution.
//         _verifyUrlsAfterSave accepts abortSignal param; all 3 call sites pass _postSaveSignal.
// v7.80 — 2026-03-31 — EO-MASTER-2b: FIX 1: EO-130g no longer mutates
//         projectData.references (React anti-pattern) — uses local copy.
//         FIX 2: EO-130i in ER composite same fix.
//         FIX 3: Step 5 fallback regex matches both [N] AND [XX-N] prefixed
//         markers — creates placeholder refs when Gemini omits _references array.
// v7.79 — 2026-03-31 — EO-MASTER: Step 7b bare-marker resolution in pipeline,
//         injectReferencesToText prompt enforces [XX-N] format with explicit forbidden list.
// v7.78 — 2026-03-31 — EO-167c: Add safeSetReferences guard function. Reference merge log updated.
//         Bug 1 (schema:) already fixed by EO-162b — confirmed 0 schema: occurrences.
//         Bug 3 (remaining chapterPrefix filters at lines ~1373, ~1549) are READ-ONLY for
...
//         Added _origRefCountForSafetyGuard + [EO-167b] safety log at STEP 6 merge.
// v7.76 — 2026-03-31 — EO-164b: Fix MODEL_OVERLOADED dialog missing + guard AutoSave language mismatch
//   Bug 3: EO-104 composite guards required e.fallbackModel != null — models with no fallback entry
//         in FALLBACK_MODELS caused ModelOverloadedError to be silently swallowed (no dialog shown).
//         FIX: removed && e.fallbackModel condition from all 6 EO-104 guards + end-of-composite check.
//         handleAIError already handles both cases (fancy dialog if fallbackModel, simple if null).
//   Bug 1: (companion fix in useProjectManager.ts v1.13) AutoSave blocked when generalObjectives
//         language detected != current language — prevents perpetuating pre-EO-164 Supabase corruption.
// v7.75 — 2026-03-30 — EO-164: Fix cross-project save corruption + SO references lost
//   FIX 1: newData.references = _refPipelineData.references moved to BEFORE validation block.
//           Root cause: specificObjectives FATAL validation (< 5 items or missing indicator)
//           caused early return, newData.references never got the 24-ref pipeline result,
//           "Save anyway" handler used empty _extractedRefs → SO refs permanently lost.
//   FIX 2: setProjectData guarded with storageService.getCurrentProjectId() check against
//           _generationStartProjectId. Prevents cross-project React state contamination
//           when user switches projects during generation. AutoSave was then persisting
//           the mixed state (English generation data) under the new project's ID (SI).
//
// ═══════════════════════════════════════════════════════════════ 
...
// ★ EO-172: Module-level repair state — persists across renders, resets per generation run.
// Prevents repairBrokenReferenceUrls from running more than once per generation regardless
// of how many times _verifyUrlsAfterSave is invoked (covers all sections: PA/GO/SO/PI/AC/PM/RS/ER).
const _repairState = {
  runId: '' as string,
  hasRun: false as boolean,
...
// ★ EO-167c: Safety guard — ensures the references array never shrinks during any merge.
// Pass existingRefs (current full array) and candidateRefs (the proposed new array).
// If candidateRefs would shrink the pool, the function appends only truly-new refs instead
// of replacing. This is the last line of defence against any code path that overwrites refs.
function safeSetReferences(existingRefs: any[], candidateRefs: any[], operation: string): any[] {
  if (candidateRefs.length < existingRefs.length) {
    console.error(
      '[EO-167c] SAFETY BLOCKED: "' + operation + '" would reduce refs from ' +
      existingRefs.length + ' to ' + candidateRefs.length + '. Merging instead of replacing.'
    );
    const merged = [...existingRefs];
    for (const nr of candidateRefs) {
      const exists = merged.some(er =>
        (er.doi && nr.doi && er.doi === nr.doi) ||
        (er.title && nr.title && er.title.toLowerCase() === nr.title.toLowerCase())
      );
...
function _runFullReferencePipeline(
  ...
  var _cleanedRefs = _currentRefs.filter(function(r: any) {
    ...
    return r.sectionKey !== sectionKey; // keep refs belonging to OTHER sections
  });
  var _removedCount = _currentRefs.length - _cleanedRefs.length;
  if (_removedCount > 0) {
    console.log('[EO-167b] Removed ' + _removedCount + ' old refs for "' + sectionKey + '" (kept ' + _cleanedRefs.length + ' from other sections)');
  ...
  if (_extractedRefs.length === 0) {
    ...
    // ★ EO-MASTER-2b FIX 3: Match BOTH bare [N] AND prefixed [XX-N] markers.
    // Old regex only matched \[(\d+)\] — missed [PA-1], [GO-3], [ER-2] etc.
    // When Gemini returns prefixed markers in text but omits _references array,
    // Step 5 must still create placeholder refs for them.
    var _markerRegexBare = /\[(\d+)\]/g;
    var _markerRegexPrefixed = /\[([A-Z]{2,3})-(\d+)\]/g;
    var _markerMatch;
    var _foundBareMarkers = new Set<string>();
    var _foundPrefixedMarkers: Array<{prefix: string; num: string; full: string}> = [];
    var _foundPrefixedSet = new Set<string>();
    // Scan for bare [N]
    while ((_markerMatch = _markerRegexBare.exec(_allGenText)) !== null) {
      _foundBareMarkers.add(_markerMatch[1]);
    }
    // Scan for prefixed [XX-N]
    while ((_markerMatch = _markerRegexPrefixed.exec(_allGenText)) !== null) {
      var _pfxFull = '[' + _markerMatch[1] + '-' + _markerMatch[2] + ']';
      if (!_foundPrefixedSet.has(_pfxFull)) {
        _foundPrefixedSet.add(_pfxFull);
        _foundPrefixedMarkers.push({prefix: _markerMatch[1], num: _markerMatch[2], full: _pfxFull});
      }
    }
    // Process bare [N] markers (original logic)
    if (_foundBareMarkers.size > 0) {
      ...
      _foundBareMarkers.forEach(function(num) {
        if (!_existingMarkers.has('[' + num + ']')) _missingMarkers.push(num);
      });
      if (_missingMarkers.length > 0) {
        console.log('[EO-MASTER-2b] FALLBACK: Creating ' + _missingMarkers.length + ' placeholder refs from bare [N] markers for "' + sectionKey + '"');
    ...
    // ★ EO-MASTER-2b FIX 3b: Process prefixed [XX-N] markers (NEW — handles Gemini returning
    // [PA-1],[PA-2] in text without _references array)
    if (_extractedRefs.length === 0 && _foundPrefixedMarkers.length > 0) {
      var _expectedPrefix = _getChapterPrefix(sectionKey);
      var _existingSectionRefsPfx = _currentRefs.filter(function(r: any) { return r.sectionKey === sectionKey; });
      var _existingMarkersPfx = new Set(_existingSectionRefsPfx.map(function(r: any) { return r.inlineMarker; }));
      var _missingPrefixedMarkers = _foundPrefixedMarkers.filter(function(pm) {
        // Only create placeholders for markers matching THIS section's expected prefix
        if (pm.prefix !== _expectedPrefix) return false;
        return !_existingMarkersPfx.has(pm.full);
      });
      if (_missingPrefixedMarkers.length > 0) {
        console.log('[EO-MASTER-2b] FALLBACK: Creating ' + _missingPrefixedMarkers.length + ' placeholder refs from prefixed [' + _expectedPrefix + '-N] markers for "' + sectionKey + '"');
        var _pfxPlaceholders: any[] = [];
        _missingPrefixedMarkers.forEach(function(pm) {
          _pfxPlaceholders.push({
            id: 'ref_' + Date.now() + '_' + Math.random().toString(36).slice(2, 7),
            sectionKey: sectionKey,
            chapterPrefix: pm.prefix,
            fieldKey: 'description',
            inlineMarker: pm.full,
            _originalMarker: pm.full,
            authors: 'Citation ' + pm.full,
            year: '',
            title: 'Reference cited as ' + pm.full + ' — needs verification',
            source: '',
            url: '',
            doi: '',
            note: 'Auto-collected from ' + pm.full + ' marker — use "Add / refresh references" to enrich',
            accessedDate: new Date().toISOString().slice(0, 10),
            addedBy: 'ai' as const,
            urlVerified: undefined,
          });
        });
        _extractedRefs = _pfxPlaceholders;
      }
    }
  }
  ...
  if (_extractedRefs.length > 0) {
    ...
    if (_deduped.length > 0) {
      var _proposedRefs = _currentRefs.concat(_deduped);
      // ★ EO-167b/167c: _proposedRefs = (_currentRefs after STEP 1) + new deduped refs.
      // This is always >= _currentRefs.length (we only ADD here).
      // If total shrank vs original, it's because STEP 1 legitimately removed stale section refs
      // AND the regeneration produced fewer citations — that is EXPECTED, not a bug.
      // ★ EO-173: Strip invalid URLs from ALL refs before merging — catches stale Scholar query
      // URLs (ROOT CAUSE 1) and Vertex AI redirect URLs (ROOT CAUSE 2) from previous saves.
      var _eo173StrippedCount = 0;
      _proposedRefs = _proposedRefs.map(function(r: any) {
        if (!r.url) return r;
        if (r.url.includes('scholar.google.com/scholar?q=') ||
            r.url.includes('vertexaisearch.cloud.google.com') ||
            r.url.includes('grounding-api-redirect')) {
          _eo173StrippedCount++;
          return { ...r, url: '', urlVerified: false, verificationStatus: 'not_found', resolvedUrl: '' };
        }
        return r;
      });
      ...
      // If approved sources were injected into the prompt, any ref URL that doesn't match
      // a known approved source URL or DOI is likely hallucinated by Gemini. Strip it.
      // Only applied to refs for the CURRENT sectionKey — other sections' refs are untouched.
      if (_opts.approvedSources && _opts.approvedSources.length > 0) {
        var _eo175rStrippedCount = 0;
        var _eo175rApprovedUrls = new Set<string>();
        var _eo175rApprovedDois = new Set<string>();
        for (var _asIdx = 0; _asIdx < _opts.approvedSources.length; _asIdx++) {
          var _asSrc = _opts.approvedSources[_asIdx];
          if (_asSrc.url) {
            _eo175rApprovedUrls.add(_asSrc.url.replace(/\/+$/, '').toLowerCase().replace(/^https?:\/\//, ''));
          }
          if (_asSrc.doi) {
            _eo175rApprovedDois.add(String(_asSrc.doi).toLowerCase().trim());
          }
        }
        _proposedRefs = _proposedRefs.map(function(r: any) {
          // Skip refs from OTHER sections — their URLs were validated against different source pools
          if (r.sectionKey && r.sectionKey !== sectionKey) return r;
          if (!r.url) return r;
          // Check direct URL match
          var _normRefUrl = r.url.replace(/\/+$/, '').toLowerCase().replace(/^https?:\/\//, '');
          if (_eo175rApprovedUrls.has(_normRefUrl)) return r;
          // Check DOI constructed URL (https://doi.org/<doi>)
          var _doiFromUrl = r.url.toLowerCase()
            .replace('https://doi.org/', '').replace('http://doi.org/', '')
            .replace('https://dx.doi.org/', '').replace('http://dx.doi.org/', '').trim();
          if (_eo175rApprovedDois.has(_doiFromUrl)) return r;
          // Check ref.doi directly
          if (r.doi && _eo175rApprovedDois.has(String(r.doi).toLowerCase().trim())) return r;
          // URL not from approved sources → likely hallucinated
          _eo175rStrippedCount++;
          console.log('[EO-175R] URL not in approved sources — stripping: ' + r.url);
          return { ...r, url: '', urlVerified: false, verificationStatus: 'not_in_approved_sources', resolvedUrl: '' };
        });
        if (_eo175rStrippedCount > 0) {
          console.warn('[EO-175R] Stripped ' + _eo175rStrippedCount + ' hallucinated URLs for "' + sectionKey + '" (approved pool: ' + _opts.approvedSources.length + ' sources)');
        } else {
          console.log('[EO-175R] All ref URLs validated against approved sources for "' + sectionKey + '" — 0 stripped ✅');
        }
      }

      newData.references = _proposedRefs;
      console.log('[EO-167b] Reference merge for "' + sectionKey + '": ' +
        _origRefCountForSafetyGuard + ' → ' + newData.references.length +
        ' (kept ' + _currentRefs.length + ' existing, added ' + _deduped.length + ' new, removed ' + _removedCount + ' stale for "' + sectionKey + '")');
      ...
      // ═══ STEP 7b: EO-MASTER — Resolve remaining bare [N] markers by text order ═══
      // Gemini uses one numbering in _references (1,2,3) and DIFFERENT numbering in text
      // (11,12,14). Step 7 replaced [1]→[PA-1] but [11] was never in _savedOriginalMarker.
      // This step maps remaining bare [N] BY ORDER OF APPEARANCE to the deduped pipeline refs.
      var _step7bTarget = newData[sectionKey];
      if (_step7bTarget && typeof _step7bTarget === 'object') {
        var _step7bJson = JSON.stringify(_step7bTarget);
        // Find all bare [N] still in text (NOT already prefixed [XX-N])
        var _step7bBareRegex = /\[(\d+)\]/g;
        var _step7bMatch;
        var _step7bBareMarkers: string[] = [];
        var _step7bSeen = new Set<string>();
        while ((_step7bMatch = _step7bBareRegex.exec(_step7bJson)) !== null) {
          var _step7bMarker = '[' + _step7bMatch[1] + ']';
          if (!_step7bSeen.has(_step7bMarker)) {
            _step7bSeen.add(_step7bMarker);
            _step7bBareMarkers.push(_step7bMarker);
          }
        }
        if (_step7bBareMarkers.length > 0 && _deduped.length > 0) {
          // Map bare markers to deduped refs by order of appearance
          var _step7bReplacements: Array<{bare: string; prefixed: string}> = [];
          for (var _step7bi = 0; _step7bi < _step7bBareMarkers.length && _step7bi < _deduped.length; _step7bi++) {
            _step7bReplacements.push({
              bare: _step7bBareMarkers[_step7bi],
              prefixed: _deduped[_step7bi].inlineMarker
            });
          }
          if (_step7bReplacements.length > 0) {
            // Sort descending by bare marker length to prevent [1] matching inside [11]
            _step7bReplacements.sort(function(a, b) { return b.bare.length - a.bare.length; });
            // Phase 1: bare → temp placeholder (prevent cross-contamination)
            var _step7bTempPrefix = '__EOMASTER_';
            for (var _step7bri = 0; _step7bri < _step7bReplacements.length; _step7bri++) {
              var _step7bEsc = _step7bReplacements[_step7bri].bare.replace(/[[\]]/g, '\\$&');
              _step7bJson = _step7bJson.replace(new RegExp(_step7bEsc, 'g'), _step7bTempPrefix + _step7bri + '__');
            }
            // Phase 2: temp placeholder → prefixed marker
            for (var _step7bri2 = 0; _step7bri2 < _step7bReplacements.length; _step7bri2++) {
              _step7bJson = _step7bJson.split(_step7bTempPrefix + _step7bri2 + '__').join(_step7bReplacements[_step7bri2].prefixed);
            }
            try {
              newData[sectionKey] = JSON.parse(_step7bJson);
              console.log('[EO-MASTER 7b] Remapped ' + _step7bReplacements.length + ' bare markers in "' + sectionKey + '": ' +
                _step7bReplacements.map(function(r) { return r.bare + '→' + r.prefixed; }).join(', '));
            } catch (_step7bParseErr) {
              console.error('[EO-MASTER 7b] JSON parse failed after replacement — keeping original');
            }
          }
        }
      }
...
function _verifyUrlsAfterSave(
  ...
  abortSignal?: AbortSignal  // ★ EO-168: kill-switch for stale async work
): void {
  if (!savedRefs || savedRefs.length === 0) return;
  // ★ EO-168: Abort check — newer generation already started
  if (abortSignal?.aborted) {
    console.log('[EO-168] _verifyUrlsAfterSave aborted — newer generation in progress');
    return;
  }

  // ★ EO-173B: Retroactive cleanup — strip Scholar query URLs and Vertex AI redirects that
  // were saved before this fix. Runs synchronously before any async verification.
  var _EO173B_INVALID_PATTERNS = [
    'scholar.google.com/scholar?q=',
    'vertexaisearch.cloud.google.com',
    'grounding-api-redirect',
  ];
  var _eo173bCleanedCount = 0;
  var _eo173bCleanedRefs = savedRefs.map(function(r: any) {
    if (!r.url) return r;
    var _isInvalid = _EO173B_INVALID_PATTERNS.some(function(p: string) { return r.url.includes(p); });
    if (_isInvalid) {
      _eo173bCleanedCount++;
      console.warn('[EO-173B] Cleaning invalid URL for [' + (r.inlineMarker || '') + '] ' + (r.title || '').substring(0, 50));
      return { ...r, url: '', urlVerified: false, verificationStatus: 'invalid_url_cleaned', verificationMethod: 'eo173_cleanup', resolvedUrl: '' };
    }
    return r;
  });
  if (_eo173bCleanedCount > 0) {
    console.log('[EO-173B] Cleaned ' + _eo173bCleanedCount + ' invalid URLs from existing references — persisting to storage');
    setProjectDataFn(function(prev: any) {
      if (!Array.isArray(prev.references)) return prev;
      var _updRefs = prev.references.map(function(existingRef: any) {
        var cleaned = _eo173bCleanedRefs.find(function(cr: any) { return cr.id === existingRef.id; });
        return (cleaned && cleaned.url !== existingRef.url) ? cleaned : existingRef;
      });
      var _updData = { ...prev, references: _updRefs };
      if (currentProjectId && storageServiceRef) {
        storageServiceRef.saveProject(_updData, language, currentProjectId)
          .catch(function(e: any) { console.warn('[EO-173B] Save after cleanup failed (non-fatal):', e?.message); });
      }
      return _updData;
    });
  }
  // Use cleaned refs as the source for URL verification (excludes newly-stripped URLs)
  var _refsForVerify = _eo173bCleanedCount > 0 ? _eo173bCleanedRefs : savedRefs;

  const toVerify = _refsForVerify.filter((r: any) => r.url && r.url.trim().length > 5);
  ...
  verifyReferencesBatch(toVerify).then(function(verResults: any[]) {
    ...
    // ★ EO-168: Second abort check after async gap
    if (abortSignal?.aborted) {
      console.log('[EO-168] _verifyUrlsAfterSave: verification returned but generation was superseded — skipping setProjectData');
      return;
    }
    ...
    // ★ EO-172: Re-enabled referenceRepair with loop prevention.
    // SAFEGUARD 3: One-shot guard — only one repair pass per generation run across ALL sections.
    // Covers: PA / GO / SO / PI / AC / PM / RS / ER — all sections that generate references.
    if (_repairState.hasRun) {
      console.log('[EO-172] referenceRepair already completed for run ' + _repairState.runId + ' — skipping (loop prevention)');
    } else {
      // Mark as run BEFORE any async work — prevents any concurrent call from also running
      _repairState.hasRun = true;

      // Collect broken refs from verification results
      const _brokenVerResults = verResults.filter(function(v: any) {
        return !v.urlVerified && (
          v.verificationStatus === 'broken' ||
          v.verificationStatus === 'not-found' ||
          v.verificationStatus === 'failed'
        );
      });

      if (_brokenVerResults.length === 0) {
        console.log('[EO-172] No broken URLs found — skipping repair');
      } else {
        // Match broken verResults back to the actual ref objects
        const _brokenRefs: any[] = _brokenVerResults
          .map(function(v: any) { return toVerify.find(function(r: any) { return r.id === v.id; }); })
          .filter(Boolean);

        // Filter out URLs already attempted (e.g. from a previous partial run)
        const _newBrokenRefs = _brokenRefs.filter(function(r: any) {
          return r.url && !_repairState.attemptedUrls.has(r.url);
        });

        if (_newBrokenRefs.length === 0) {
          console.log('[EO-172] All broken URLs already attempted — skipping');
        } else {
          // SAFEGUARD 4: Mark URLs as attempted BEFORE calling repair — prevents retry on re-entry
          _newBrokenRefs.forEach(function(r: any) { _repairState.attemptedUrls.add(r.url); });

          console.log('[EO-172] Repairing ' + _newBrokenRefs.length + ' broken URLs (run: ' + _repairState.runId + ')');

          // Kick off repair — fire-and-forget (non-blocking for the UI)
          repairBrokenReferenceUrls(_newBrokenRefs, language).then(function(repairedRefs: any[]) {
            const groundingFixed = repairedRefs.filter(function(r: any) { return r.verificationMethod === 'google-search-grounding'; }).length;
            const scholarFixed = repairedRefs.filter(function(r: any) { return r.verificationMethod === 'google-scholar-fallback'; }).length;
            const unfixable = _newBrokenRefs.length - groundingFixed - scholarFixed;
            console.log('[EO-172] Repair complete: ' + groundingFixed + ' via grounding, ' + scholarFixed + ' via Scholar, ' + unfixable + ' unfixable');

            if (repairedRefs.length === 0) {
              console.log('[EO-172] No URLs were fixable — nothing to save');
              return;
            }

            // SAFEGUARD 4 (save): Apply via setProjectData so React state + storage both update.
            // hasRun=true above ensures this callback cannot re-trigger another repair pass.
            setProjectDataFn(function(prev: any) {
              if (!Array.isArray(prev.references)) return prev;
              const updatedRefs = prev.references.map(function(existingRef: any) {
                const repairedRef = repairedRefs.find(function(rr: any) { return rr.id === existingRef.id; });
                return repairedRef || existingRef;
              });
              const updatedData = { ...prev, references: updatedRefs };
              if (currentProjectId && storageServiceRef) {
                storageServiceRef.saveProject(updatedData, language, currentProjectId)
                  .catch(function(e: any) { console.warn('[EO-172] Save after repair failed (non-fatal):', e?.message); });
              }
              console.log('[EO-172] Repaired refs saved (' + repairedRefs.length + ' refs updated). Total refs: ' + updatedRefs.length);
              return updatedData;
            });
          }).catch(function(repairErr: any) {
            console.warn('[EO-172] Reference repair failed (non-fatal):', repairErr?.message || repairErr);
          });
        }
      }
    }
    ...
    // ★ EO-168: Final abort check before mutating state
    if (abortSignal?.aborted) {
      console.log('[EO-168] _verifyUrlsAfterSave: aborted before URL-status setProjectData');
      return;
    }
    setProjectDataFn(function(prev: any) {
      if (!Array.isArray(prev.references)) return prev;
...
function _convertAiRefsToReferences(aiRefs: any[], sectionKey: string, existingRefs: any[]): any[] {
  ...
  for (var i = 0; i < aiRefs.length; i++) {
    ...
    // ★ EO-173: Filter Scholar search query URLs — not a valid source URL
    if (ar.url && ar.url.includes('scholar.google.com/scholar?q=')) {
      console.warn('[EO-173] Stripped Scholar search query URL (not a source URL) for: ' + (ar.title || '').substring(0, 50));
      ar.url = '';
      if (ar.urlVerified) ar.urlVerified = false;
      if (ar.verificationStatus === 'scholar-fallback') ar.verificationStatus = 'not_found';
    }
...
export async function injectReferencesToText(
  ...
  // ★ EO-MASTER B: Explicit [XX-N] format enforcement — prevents bare [N] from Gemini
  const prompt = [
    ...
    'CHAPTER PREFIX: "' + chapterPrefix + '"',
    'ADD inline citations ONLY in this EXACT format:',
    '  (Author/Institution, Year) [' + chapterPrefix + '-N]',
    'Start numbering at [' + chapterPrefix + '-' + nextNum + '], then [' + chapterPrefix + '-' + (nextNum + 1) + '], etc.',
    '',
    'FORBIDDEN:',
    '- Bare [1], [2], [11] etc. without the "' + chapterPrefix + '-" prefix',
    '- (Author, Year) alone without [' + chapterPrefix + '-N] after it',
    '- Any numbering format other than [' + chapterPrefix + '-N]',
    '',
    'CORRECT examples:',
    '- "...circular economy (European Commission, 2024) [' + chapterPrefix + '-' + nextNum + ']."',
    '- "...manufacturing sector (Urbinati et al., 2025) [' + chapterPrefix + '-' + (nextNum + 1) + ']."',
    '',
    'Aim for 1-3 citations per paragraph. DO NOT cite project decisions or purely descriptive technical text.',
    '',
    'For each citation, include a _references array entry with these fields:',
    '  inlineMarker: "[' + chapterPrefix + '-N]"  (MUST match the marker used in the text)',
    '  authors, year, title, source, url, doi',
    '',
    'The inlineMarker field in each _references entry MUST use the SAME [' + chapterPrefix + '-N] format as in the text.',
    '',
...
export const useGeneration = ({
  ...
  // ★ EO-168: AbortController for post-save async processes (URL verify, repair)
  // Aborted at the start of each new executeGeneration call to kill stale verification loops
  const _asyncPostSaveAbortController = useRef<AbortController | null>(null);
...
// ─── Execute section generation ────────────────────────────────
    ...
    async (sectionKey: string, mode: string = 'regenerate', repairInstructions?: string) => {
      ...
      // ★ EO-168: Abort any stale post-save async processes from a previous generation
      if (_asyncPostSaveAbortController.current) {
        _asyncPostSaveAbortController.current.abort();
        console.log('[EO-168] Aborted previous async post-save processes (URL verify / repair)');
      }
      _asyncPostSaveAbortController.current = new AbortController();
      const _postSaveSignal = _asyncPostSaveAbortController.current.signal;

      // ★ EO-172: Reset repair state for this generation run — one repair pass per run, all sections
      _repairState.runId = (typeof crypto !== 'undefined' && crypto.randomUUID) ? crypto.randomUUID() : Math.random().toString(36).slice(2);
      _repairState.hasRun = false;
      _repairState.attemptedUrls.clear();
      console.log('[EO-172] Repair state reset for new generation run:', _repairState.runId);
      ...
      try {
        ...
        const APPROVED_SOURCE_SECTIONS = new Set([
          ...
          'generalObjectives', 'specificObjectives',
          'outputs', 'outcomes', 'impacts', 'kers',
          // EO-174: risks and projectManagement now use approved sources in standalone regeneration
          'risks', 'projectManagement',
          // EO-174b: activities now fetches approved sources — passed to generateActivitiesPerWP
          'activities',
        ]);
        ...
        if (subMapping) {
          ...
          if (mode === 'regenerate' || existingWPs.length === 0) {
            generatedData = await generateActivitiesPerWP(
              ...
              undefined,
              signal,
              // ★ EO-174b: inject approved sources + refs flag (both computed above by APPROVED_SOURCE_SECTIONS fetch)
              _approvedSourcesBlock || undefined,
              _execRefsEnabled
            );
            ...
            generatedData = await generateActivitiesPerWP(
              ...
              signal,
              _approvedSourcesBlock || undefined, // ★ EO-174b
              _execRefsEnabled                    // ★ EO-174b
            );
        ...
        // ★ EO-MASTER-2b FIX 1: NEVER mutate projectData (React state prop).
        // Previous code did (projectData as any).references = ... which mutated React state in-place.
        // This caused pipeline to work on corrupted state → references lost after save.
        // FIX: Create local cleaned copy, pass to pipeline. projectData stays untouched.
        var _cleanedRefsForPipeline: any[] = [];
        if (projectData?.references && Array.isArray(projectData.references)) {
          const _preCleanBefore = projectData.references.length;
          _cleanedRefsForPipeline = projectData.references.filter((ref: any) => {
            const refSection = ref.sectionKey || '';
            return !_regenChildKeys.includes(refSection);
          });
          const _preCleanRemoved = _preCleanBefore - _cleanedRefsForPipeline.length;
          console.log(`[EO-MASTER-2b] Pre-pipeline cleanup (LOCAL copy): removed ${_preCleanRemoved} old refs for chapter "${_regenChapterKey}" (section: "${sectionKey}"). Kept: ${_cleanedRefsForPipeline.length}. projectData.references UNTOUCHED: ${projectData.references.length}`);
        }

        // Initialize pipeline with CLEANED LOCAL copy — projectData.references is NOT mutated
        var _refPipelineData: any = { references: _cleanedRefsForPipeline.slice(), [sectionKey]: generatedData };
        ...
        // ★ EO-164 FIX 1: Sync reference pipeline results into newData BEFORE validation.
        // ROOT CAUSE of SO refs lost: previously this sync was at line ~4157 (AFTER validation).
        // specificObjectives validation has FATAL conditions (< 5 items, missing indicator).
        // When FATAL triggered → early return at line 4082 → line 4157 never executed →
        // newData.references stayed at 20 refs (from { ...projectData }) → SO refs never saved.
        // "Save anyway" handler used _extractedRefs (reset to [] at line 3473) → also lost.
        // FIX: sync here so ALL save paths (normal, "Save anyway", early return) have correct refs.
        // Also captures any refs added by subMapping extraction (lines 3488-3501) above.
        newData.references = _refPipelineData.references;
        console.log('[EO-164] newData.references synced from pipeline: ' + newData.references.length + ' refs (sectionKey="' + sectionKey + '", before validation)');
        ...
        // ★ EO-100 / EO-164: newData.references already synced above (before validation) — no-op here.
        // Previously: newData.references = _refPipelineData.references; ← moved to before validation by EO-164
        ...
        // ★ EO-164 FIX 2: Guard against cross-project state contamination.
        // ROOT CAUSE of English data saved into SI projects: when user switches projects
        // during generation, setProjectData below receives prev = NEW project's data,
        // and spreads newData (from OLD project's generation) on top → MIXED state.
        // AutoSave then fires with this mixed state under the NEW project's ID → corruption.
        // _guardedSaveProject already blocks the explicit DB save, but setProjectData itself
        // had no guard — it was silently contaminating React state even when the project changed.
        // FIX: check storageService.getCurrentProjectId() (always up-to-date via sessionStorage)
        // against _generationStartProjectId. If mismatch → skip setProjectData entirely.
        const _eo164LiveProjectId = storageService.getCurrentProjectId();
        if (_eo164LiveProjectId && _eo164LiveProjectId !== _generationStartProjectId) {
          console.error('[EO-164] setProjectData BLOCKED — project switched during generation!', {
            startedFor: _generationStartProjectId,
            currentActive: _eo164LiveProjectId,
            sectionKey,
            language,
            refsCount: newData.references?.length,
          });
          console.error('[EO-164] Generation result for "' + sectionKey + '" discarded. React state preserved for "' + _eo164LiveProjectId + '". User can regenerate after switching back.');
          // Return without calling setProjectData — finally block handles cleanup.
          return;
        }
        ...
        setProjectData((prev: any) => {
          ...
          if (currentProjectId) {
            _guardedSaveProject(savedData, language, currentProjectId)
              .then(() => {
                console.log('[EO-164][executeGeneration] ★ Explicit save after ' + sectionKey + ' — lang=' + language + ', projectId=' + currentProjectId + ', generalObjectives: ' + (Array.isArray(savedData.generalObjectives) && savedData.generalObjectives.some((o: any) => o.title?.trim()) ? '✅ HAS' : '⚠️ EMPTY') + ', refs: ' + (Array.isArray(savedData.references) ? savedData.references.length : 0));
        ...
        if (!_verifyRefsEnabled) {
          ...
          _verifyUrlsAfterSave(newData.references, setProjectData, currentProjectId, language, storageService, _postSaveSignal);  // ★ EO-168
  ...
  const handleGenerateCompositeSection = useCallback(
    async (compositeSectionKey: string) => {
      ...
      // ── Main composite runner ──
        ...
        // ★ EO-168b: _approvedSources was declared in executeGeneration scope and was NOT accessible
        // here. Now changed to let so EO-174 can accumulate PM + risks approved sources into it,
        // which the BUG29 persist block (~line 5798) then writes to newData.approvedSources.
        // EO-159 BUG29 persist blocks still skip gracefully when length=0 (ER composite path).
        let _approvedSources: any[] = [];

        // ★ EO-168c PROBLEM 1: _postSaveSignal is declared in executeGeneration scope and is NOT
        // accessible here. runComposite manages its own AbortController (signal) for the main
        // generation. For the post-save verify call we pass undefined — _verifyUrlsAfterSave
        // treats undefined signal as "no abort" which is safe for composite saves.
        const _postSaveSignal: AbortSignal | undefined = undefined;

        // ★ EO-172: Reset repair state for this composite run
        _repairState.runId = (typeof crypto !== 'undefined' && crypto.randomUUID) ? crypto.randomUUID() : Math.random().toString(36).slice(2);
        _repairState.hasRun = false;
        _repairState.attemptedUrls.clear();
        ...
        try {
          ...
          if (mode === 'regenerate' && currentProjectId) {
            ...
            _updatePhase('step1_pm', 'running');
              ...
              try {
              // ★ EO-174: Fetch approved sources (CrossRef + OpenAlex) for projectManagement
              // before generation so Gemini cites real papers instead of hallucinating URLs.
              var _pmApprovedSources: any[] = [];
              var _pmApprovedSourcesBlock: string = '';
              if (_compRefsEnabled) {
                try {
                  var _pmSearchQuery = buildSearchQuery('projectManagement', newData);
                  if (_pmSearchQuery && _pmSearchQuery.length >= 5) {
                    console.log('[EO-174] Searching approved sources for "projectManagement": "' + _pmSearchQuery.substring(0, 80) + '..."');
                    _pmApprovedSources = await searchAuthoritativeSources(_pmSearchQuery, 'projectManagement');
                    if (_pmApprovedSources.length > 0) {
                      _pmApprovedSourcesBlock = formatApprovedSourcesForPrompt(_pmApprovedSources);
                      _approvedSources = [..._approvedSources, ..._pmApprovedSources];
                      console.log('[EO-174] Injecting ' + _pmApprovedSources.length + ' approved sources into PM prompt');
                    } else {
                      console.log('[EO-174] No approved sources found for projectManagement — proceeding without');
                    }
                  }
                } catch (_pmSrcErr) {
                  console.warn('[EO-174] Approved sources search failed for projectManagement (non-fatal):', _pmSrcErr);
                }
              }
              var pmContentRaw = await generateSectionContent(
                'projectManagement', newData, language, mode, null, signal, _pmApprovedSourcesBlock || undefined, _compRefsEnabled
              );
              ...
              // EO-170 Part D: Pre-merge debug
              console.log('[EO-170] PM PRE merge: has [PM-] markers=' + JSON.stringify(pmContent || '').includes('[PM-'));
              newData.projectManagement = {
                ...newData.projectManagement,
                ...pmContent,
                structure: {
              ...
              // EO-170 Part D: Post-merge debug
              console.log('[EO-170] PM POST merge: has [PM-] markers=' + JSON.stringify(newData.projectManagement || '').includes('[PM-'));
              ...
              if (_refsEnabled130h) {
                ...
                var _pmRefResult = _runFullReferencePipeline('projectManagement', _pmDataWithRefs, newData, {
                  ...
                  approvedSources: _pmApprovedSources.length > 0 ? _pmApprovedSources : undefined, // ★ EO-175R
                });
                ...
                console.log('[EO-170] PM POST pipeline: has [PM-] markers=' + JSON.stringify(newData.projectManagement || '').includes('[PM-'));
              } else {
              ...
              // ★ EO-164b Bug 3: Removed && e.fallbackModel — handleAIError handles both cases (with/without fallback model)
              if (e.name === 'ModelOverloadedError') {
                console.warn('[EO-104][EO-164b] ModelOverloadedError at Step 1 (PM) — showing dialog (fallbackModel=' + (e.fallbackModel ? e.fallbackModel.id : 'none') + ')');
            ...
            _updatePhase('step2_partners', 'running'); // [EO-137b]
              ...
              // ★ EO-164b Bug 3: Removed && e.fallbackModel — handleAIError handles both cases
              if (e.name === 'ModelOverloadedError') {
                console.warn('[EO-104][EO-164b] ModelOverloadedError at Step 2 (Partners) — showing dialog (fallbackModel=' + (e.fallbackModel ? e.fallbackModel.id : 'none') + ')');
                if (!firstFatalError) firstFatalError = e;
                setProjectData(function(prev: any) {
                  var savedData = Object.assign({}, prev, newData);
                  if (currentProjectId) {
            ...
            // ★ EO-174b: Fetch approved sources for activities BEFORE generateActivitiesPerWP.
            // Called once here; passed to every per-WP prompt so Gemini cites real papers.
            var _acApprovedSources: any[] = [];
            var _acApprovedSourcesBlock: string = '';
            if (_refsEnabled130h) {
              try {
                var _acSearchQuery = buildSearchQuery('activities', newData);
                if (_acSearchQuery && _acSearchQuery.length >= 5) {
                  console.log('[EO-174b] Searching approved sources for "activities": "' + _acSearchQuery.substring(0, 80) + '..."');
                  _acApprovedSources = await searchAuthoritativeSources(_acSearchQuery, 'activities');
                  if (_acApprovedSources.length > 0) {
                    _acApprovedSourcesBlock = formatApprovedSourcesForPrompt(_acApprovedSources);
                    _approvedSources = [..._approvedSources, ..._acApprovedSources];
                    console.log('[EO-174b] Injecting ' + _acApprovedSources.length + ' approved sources into activities prompt');
                  } else {
                    console.log('[EO-174b] No approved sources found for activities — proceeding without');
                  }
                }
              } catch (_acSrcErr) {
                console.warn('[EO-174b] Approved sources search failed for activities (non-fatal):', _acSrcErr);
              }
            }
            ...
            try {
              ...
              if (mode === 'regenerate' || existingWPs.length === 0) {
                activitiesResult = await generateActivitiesPerWP(
                  ...
                  _acApprovedSourcesBlock || undefined, // ★ EO-174b
                  _refsEnabled130h                      // ★ EO-174b
                );
              } else if (mode === 'enhance') {
                  ...
                  for (var _wpRefIdx = 0; _wpRefIdx < newData.activities.length; _wpRefIdx++) {
                    ...
                    if (_wpForRefs && typeof _wpForRefs === 'object') {
                      ...
                      var _wpRefResult = _runFullReferencePipeline(_wpSectionKey, _wpForRefs, newData, {
                        setProjectData: setProjectData,
                        skipVerification: true,
                        approvedSources: _acApprovedSources.length > 0 ? _acApprovedSources : undefined, // ★ EO-175R
                      });
              ...
              // ★ EO-164b Bug 3: Removed && e.fallbackModel — handleAIError handles both cases
              if (e.name === 'ModelOverloadedError') {
                console.warn('[EO-104][EO-164b] ModelOverloadedError at Step 3 (Activities) — showing dialog (fallbackModel=' + (e.fallbackModel ? e.fallbackModel.id : 'none') + ')');
                if (!firstFatalError) firstFatalError = e;
                setProjectData(function(prev: any) {
                  var savedData = Object.assign({}, prev, newData);
                  if (currentProjectId) {
            ...
            if (pa_partners.length > 0 && pa_activities.length > 0) {
              ...
              try {
                ...
                // ★ EO-164b Bug 3: Removed && e.fallbackModel — handleAIError handles both cases
                if (e.name === 'ModelOverloadedError') {
                  console.warn('[EO-104][EO-164b] ModelOverloadedError at Step 4 (Allocations) — showing dialog (fallbackModel=' + (e.fallbackModel ? e.fallbackModel.id : 'none') + ')');
                  if (!firstFatalError) firstFatalError = e;
                  setProjectData(function(prev: any) {
                    var savedData = Object.assign({}, prev, newData);
                    if (currentProjectId) {
            ...
            _updatePhase('step5_risks', 'running'); // [EO-137b]
                        ...
                        try {
              // ★ EO-174: Fetch approved sources (CrossRef + OpenAlex) for risks before generation
              var _rsApprovedSources: any[] = [];
              var _rsApprovedSourcesBlock: string = '';
              if (_compRefsEnabled) {
                try {
                  var _rsSearchQuery = buildSearchQuery('risks', newData);
                  if (_rsSearchQuery && _rsSearchQuery.length >= 5) {
                    console.log('[EO-174] Searching approved sources for "risks": "' + _rsSearchQuery.substring(0, 80) + '..."');
                    _rsApprovedSources = await searchAuthoritativeSources(_rsSearchQuery, 'risks');
                    if (_rsApprovedSources.length > 0) {
                      _rsApprovedSourcesBlock = formatApprovedSourcesForPrompt(_rsApprovedSources);
                      _approvedSources = [..._approvedSources, ..._rsApprovedSources];
                      console.log('[EO-174] Injecting ' + _rsApprovedSources.length + ' approved sources into risks prompt');
                    } else {
                      console.log('[EO-174] No approved sources found for risks — proceeding without');
                    }
                  }
                } catch (_rsSrcErr) {
                  console.warn('[EO-174] Approved sources search failed for risks (non-fatal):', _rsSrcErr);
                }
              }
            var risksContentRaw = await generateSectionContent(
                'risks', newData, language, mode, null, signal, _rsApprovedSourcesBlock || undefined, _compRefsEnabled
              );
              ...
              // ★ EO-102B: Step B — Unwrap FIRST
              // EO-170 FIX: The risks schema (EO-159 BUG12) wraps the array under "items" key,
              // NOT "risks". Gemini returns { items: [...], _references: [...] }. Step B must
              // handle BOTH formats: { risks: [...] } (legacy) and { items: [...] } (schema-correct).
              var risksContent = risksContentRaw;
              if (risksContent && typeof risksContent === 'object' && !Array.isArray(risksContent)) {
                if (Array.isArray((risksContent as any).risks)) {
                  // Legacy format: { risks: [...] }
                  risksContent = (risksContent as any).risks;
                  console.log('[EO-170] Risks: unwrapped { risks: [...] } format');
                } else if (Array.isArray((risksContent as any).items)) {
                  // Schema-correct format (EO-159 BUG12): { items: [...] }
                  risksContent = (risksContent as any).items;
                  console.log('[EO-170] Risks: unwrapped { items: [...] } format (schema-correct)');
                }
              }

              // EO-170 Part D: Debug — log whether inline [RS-] markers are present
              var _risksPreCheck = JSON.stringify(risksContent || '');
              console.log('[EO-170] Risks PRE format-check: isArray=' + Array.isArray(risksContent) +
                ' | has [RS-] markers=' + _risksPreCheck.includes('[RS-'));
              ...
              if (Array.isArray(risksContent)) {
                ...
                console.warn('[EO-170] Risks Step B failed — risksContentRaw keys: ' + Object.keys(risksContentRaw || {}).join(', '));
              }

              // EO-170 Part D: Post format-check
              console.log('[EO-170] Risks POST format-check: has [RS-] markers=' + JSON.stringify(newData.risks || '').includes('[RS-'));
              ...
              if (_refsEnabled130h && Array.isArray(newData.risks) && _risksExtractedAiRefs.length > 0) {
                ...
                var _risksRefResult = _runFullReferencePipeline('risks', _risksWrapper, newData, {
                  setProjectData: setProjectData,
                  skipVerification: true, // EO-118: skip async URL verify inside composite — prevents post-renumber overwrite
                  approvedSources: _rsApprovedSources.length > 0 ? _rsApprovedSources : undefined, // ★ EO-175R
                });
              ...
              // ★ EO-164b Bug 3: Removed && e.fallbackModel — handleAIError handles both cases
              if (e.name === 'ModelOverloadedError') {
                console.warn('[EO-104][EO-164b] ModelOverloadedError at Step 5 (Risks) — showing dialog (fallbackModel=' + (e.fallbackModel ? e.fallbackModel.id : 'none') + ')');
                if (!firstFatalError) firstFatalError = e;
                setProjectData(function(prev: any) {
                  var savedData = Object.assign({}, prev, newData);
                  if (currentProjectId) {
    ...
    setModalConfig({
      ...
      onCancel: closeModal,
            ...
            if (!_refsEnabled130h) {
              ...
              // ★ EO-175R-P4: EO-152 auto-repair DISABLED for Activities composite.
              // Same root cause as ER: approved-sources pipeline intentionally leaves URL empty.
              // Standalone path does NOT have EO-152 and works correctly.
              console.log('[EO-175R-P4] EO-152 auto-repair DISABLED for Activities composite — empty-URL refs are intentional.');
            } else {
              _updatePhase('urlVerification', 'skipped'); // [EO-137b]
            }
            setHasUnsavedTranslationChanges(true);
            console.log(`[Composite/activities] DONE — ${successCount}/${totalSteps} steps succeeded ✅`);
              ...
              // ★ EO-099 / EO-164b Bug 3: Delegate to handleAIError for ModelOverloadedError.
              // Replaces old inline setModalConfig that required fallbackModel != null — if the current
              // model had no entry in FALLBACK_MODELS, this block was skipped → fell to generic dialog.
              // handleAIError already handles both cases (fancy switch dialog if fallbackModel, simple if null).
              if (firstFatalError.name === 'ModelOverloadedError') {
                console.warn('[EO-164b] Partial activities composite: ' + successCount + '/' + totalSteps + ' succeeded — ModelOverloadedError (fallbackModel=' + (firstFatalError.fallbackModel ? firstFatalError.fallbackModel.id : 'none') + '). Delegating to handleAIError.');
                handleAIError(firstFatalError, 'compositeActivities');
            ...
            // no approvedSourcesBlock → Gemini hallucinates every URL (google_search is disabled
            // in JSON mode per EO-135, so it has no other way to find real sources).
            var _erApprovedSources: any[] = [];
            var _erApprovedSourcesBlock: string = '';
            if (_compRefsEnabled) {
              try {
                var _erSearchQuery = buildSearchQuery('expectedResults', projectData);
                if (_erSearchQuery && _erSearchQuery.length >= 5) {
                  console.log('[EO-175R] Searching approved sources for expectedResults: "' + _erSearchQuery.substring(0, 80) + '..."');
                  _erApprovedSources = await searchAuthoritativeSources(_erSearchQuery, 'expectedResults');
                  if (_erApprovedSources.length > 0) {
                    _erApprovedSourcesBlock = formatApprovedSourcesForPrompt(_erApprovedSources);
                    _approvedSources = [..._approvedSources, ..._erApprovedSources];
                    console.log('[EO-175R] Injecting ' + _erApprovedSources.length + ' approved sources into ER prompts');
                  } else {
                    console.log('[EO-175R] No approved sources found for expectedResults — proceeding without');
                  }
                }
              } catch (_erSrcErr) {
                console.warn('[EO-175R] Approved sources fetch failed for ER (non-fatal):', _erSrcErr);
              }
            }
                ...
                try {
                  ...
                  if (_compRefsEnabled) {
                    ...
                    var _compRefResult = _runFullReferencePipeline(s, generatedData, _compTempData, {
                      ...
                      approvedSources: _erApprovedSources.length > 0 ? _erApprovedSources : undefined, // ★ EO-175R PATCH 1
                    });
                  ...
                  // ★ EO-164b Bug 3: Removed && e.fallbackModel — handleAIError handles both cases
                  if (e.name === 'ModelOverloadedError') {
                    console.warn('[EO-104][EO-164b] ModelOverloadedError in expectedResults/' + s + ' — showing dialog (fallbackModel=' + (e.fallbackModel ? e.fallbackModel.id : 'none') + ')');
                    ...
                    _guardedSaveComposite(renumData, language, currentProjectId)
                      .then(() => {
                        ...
                        if (Array.isArray(renumData.references) && renumData.references.length > 0) {
                          _updatePhase('urlVerification', 'running'); // [EO-137b]
                          _verifyUrlsAfterSave(renumData.references, setProjectData, currentProjectId, language, storageService, _postSaveSignal);  // ★ EO-168
                          setTimeout(() => _updatePhase('urlVerification', 'completed'), 500); // [EO-137b]
                          // ★ EO-175R-P4: EO-152 auto-repair DISABLED for ER composite.
                          // ROOT CAUSE: Approved-sources pipeline intentionally leaves URL empty
                          // when no matching source exists in the approved pool (verificationStatus:
                          // 'not_in_approved_sources'). These are NOT broken — they are correctly
                          // processed. Standalone path (PA/GO/SO/PI) does NOT have EO-152 and works
                          // correctly — repair runs only for verified-then-broken URLs via EO-172
                          // inside _verifyUrlsAfterSave. EO-152 was triggering 2×14 refs × 4 retries
                          // × 20s backoff = 18 minutes of wasted API calls with 0% success rate.
                          // User can manually repair via the UI "Repair references" button.
                          console.log('[EO-175R-P4] EO-152 auto-repair DISABLED for ER composite — empty-URL refs are intentional (approved-sources pipeline). Use manual repair button if needed.');
                        } else {
                          _updatePhase('urlVerification', 'skipped'); // [EO-137b]
                        }
                      })
...
export async function repairBrokenReferenceUrls(
  ...
  // ★ EO-175R PATCH 2 CHANGE B: Circuit breaker — stop retrying when model is overloaded.
  // After REPAIR_MAX_CONSECUTIVE_OVERLOADED consecutive 503/overloaded responses the loop
  // marks all remaining refs as 'manual-review' without making any further API calls.
  var REPAIR_MAX_CONSECUTIVE_OVERLOADED = 3;
  var consecutiveOverloaded = 0;
  var circuitBroken = false;

  // Sequential (not parallel) — rate limit protection per spec
  for (var refIdx = 0; refIdx < brokenRefs.length; refIdx++) {
    var ref = brokenRefs[refIdx];

    // ★ EO-175R-P2: If circuit is broken skip all remaining refs immediately.
    if (circuitBroken) {
      console.log(`[EO-175R-P2] Circuit broken — marking "${(ref.title || '').substring(0, 50)}" as manual-review (skipped)`);
      repaired.push({
        ...ref,
        url: ref.url || '',
        urlVerified: false,
        verificationStatus: 'manual-review',
        verificationMethod: 'circuit_breaker_stop',
        resolvedUrl: '',
      });
      continue;
    }
    ...
    try {
      ...
      // Successful API call — reset overloaded counter.
      ...
      try {
        ...
        // unparseable — fall through to not_found
      }

      if (parsed?.url) {
        // ★ EO-173 ROOT CAUSE 2: Vertex AI grounding-api-redirect URLs are internal and
        // inaccessible to end users. Strip them before accepting — fall through to not_found.
        const _isVertexRedirect = parsed.url.includes('vertexaisearch.cloud.google.com') ||
          parsed.url.includes('grounding-api-redirect');
        if (_isVertexRedirect) {
          console.warn(`[EO-173] Vertex AI redirect URL stripped for "${(ref.title || '').substring(0, 50)}": ${parsed.url}`);
          // Fall through — groundingSucceeded remains false
        } else {
          ...
          if (vr?.urlVerified) {
            ...
            console.log(`[EO-148] "${ref.title}" → Google Search grounding → URL not verified → not_found`);
          }
        }
      } else {
        console.log(`[EO-148] "${ref.title}" → Google Search grounding → NOT FOUND → not_found`);
      }
    } catch (err: any) {
      const errMsg = (err?.message || String(err)).toLowerCase();
      // ★ EO-175R-P2: Detect overloaded / 503 errors and increment circuit-breaker counter.
      const isOverloaded =
        errMsg.includes('overloaded') ||
        errMsg.includes('503') ||
        errMsg.includes('service unavailable') ||
        errMsg.includes('model_overloaded') ||
        errMsg.includes('resource_exhausted');

      if (isOverloaded) {
        consecutiveOverloaded++;
        console.warn(
          `[EO-175R-P2] Overloaded response for "${(ref.title || '').substring(0, 50)}" ` +
          `(${consecutiveOverloaded}/${REPAIR_MAX_CONSECUTIVE_OVERLOADED})`
        );
        if (consecutiveOverloaded >= REPAIR_MAX_CONSECUTIVE_OVERLOADED) {
          circuitBroken = true;
          console.error(
            `[EO-175R-P2] CIRCUIT BREAKER TRIGGERED after ${consecutiveOverloaded} consecutive ` +
            `overloaded errors — stopping repair, marking remaining refs as manual-review`
          );
        }
      } else {
        // Isolated failure (network, parse, etc.) — reset overloaded streak.
        consecutiveOverloaded = 0;
    ...
    if (!groundingSucceeded) {
      // ★ EO-173 ROOT CAUSE 1: Do NOT store scholar.google.com/scholar?q= as ref.url.
      // A Scholar SEARCH QUERY is not a valid source URL — it is a search interface URL that
      // changes results over time and does not point to the actual document.
      // Old behaviour (urlVerified: true on Scholar search link) was misleading users into
      // thinking the reference was valid. Now mark as not_found so the badge shows correctly.
      console.log(`[EO-148] "${(ref.title || '').substring(0, 60)}" → ${circuitBroken ? 'manual-review (circuit broken)' : 'not_found (no valid URL located)'}`);
      repairedRef = {
        ...repairedRef,
        url: '',
        urlVerified: false,
        verificationStatus: circuitBroken ? 'manual-review' : 'not_found',
        verificationMethod: circuitBroken ? 'circuit_breaker_stop' : 'scholar_search_failed',
        resolvedUrl: '',
      };
    }
  ...
  const scholarFailed = repaired.filter(r => r.verificationMethod === 'scholar_search_failed').length;
  const circuitStopped = repaired.filter(r => r.verificationMethod === 'circuit_breaker_stop').length;
  console.log(
    `[EO-148] Repair complete: ${groundingFixed} fixed via grounding, ` +
    `${scholarFailed} not_found, ${circuitStopped} circuit-breaker-stopped (manual-review)`
  );
