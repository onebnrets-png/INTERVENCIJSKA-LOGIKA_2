...
import { getChapterPrefix as _appGetChapterPrefix, normalizeUrlForMatch as _appNormalizeUrl } from './utils/referencePrefixMap.ts'; // ★ EO-159
...
const App = () => {
  ...
  // ★ EO-159 BUG 23: Migrate legacy [N] markers to [XX-N] on project load
  useEffect(() => {
    if (!pm.currentProjectId || !pm.projectData?.references) return;
    const refs = pm.projectData.references;
    if (!Array.isArray(refs) || refs.length === 0) return;

    const needsMigration = refs.some((r: any) =>
      r.inlineMarker && /^\[\d+\]$/.test(r.inlineMarker) && !r.chapterPrefix
    );

    if (needsMigration) {
      console.log('[EO-159 BUG23] Legacy [N] markers detected, migrating...');
      const migrated = refs.map((ref: any) => {
        const updated = { ...ref };
        if (!updated.chapterPrefix && updated.sectionKey) {
          updated.chapterPrefix = _appGetChapterPrefix(updated.sectionKey);
        }
        if (updated.inlineMarker && /^\[\d+\]$/.test(updated.inlineMarker) && updated.chapterPrefix) {
          const num = updated.inlineMarker.match(/\d+/)?.[0] || '1';
          updated.inlineMarker = `[${updated.chapterPrefix}-${num}]`;
        }
        return updated;
      });
      pm.handleUpdateData(['references'], migrated);
      console.log('[EO-159 BUG23] Migrated ' + migrated.length + ' references to prefixed format');
    }
  }, [pm.currentProjectId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // ★ EO-159 BUG 7: Renumber after adding + verify URL
  const handleAddReference = async (ref: any) => {
    ...
    if (!newRef.chapterPrefix && newRef.sectionKey) {
      newRef.chapterPrefix = _appGetChapterPrefix(newRef.sectionKey);
    }

    const allRefs = [...currentRefs, newRef];

    // Renumber all refs with same chapterPrefix
    const prefix = newRef.chapterPrefix || 'REF';
    let counter = 1;
    const renumbered = allRefs.map((r: any) => {
      if ((r.chapterPrefix || 'REF') === prefix) {
        return { ...r, inlineMarker: `[${prefix}-${counter++}]` };
      }
      return r;
    });

    pm.handleUpdateData(['references'], renumbered);
    setAddRefModalOpen(false);
    console.log('[EO-159 BUG7] Added reference with marker [' + prefix + '-' + (counter - 1) + ']');

    // Fire-and-forget URL verification
    if (newRef.url) {
      try {
        const { verifyReferencesBatch } = await import('./services/referenceVerificationService.ts');
        verifyReferencesBatch([newRef]).catch(() => {});
      } catch (_e) { /* non-critical */ }
    }
  };

  // ★ EO-159 BUG 8: Reset verification on URL/DOI change + re-verify async
  const handleEditReference = async (id: string, updates: any) => {
    const currentRefs = Array.isArray(pm.projectData.references) ? pm.projectData.references : [];
    const oldRef = currentRefs.find((r: any) => r.id === id);
    ...
    const updatedRefs = currentRefs.map((r: any) => {
      ...
      if (updates.url !== undefined && updates.url !== r.url) {
        updated.urlVerified = undefined;
        updated.verificationStatus = 'pending';
        updated.verificationMethod = undefined;
        updated.resolvedUrl = undefined;
        updated.canonicalUrl = undefined;
        updated.metadataVerified = undefined;
        console.log('[EO-159 BUG8] URL changed for ' + (r.inlineMarker || r.id) + ', reset verification');
      }
      if (updates.doi !== undefined && updates.doi !== r.doi) {
        updated.urlVerified = undefined;
        updated.verificationStatus = 'pending';
      }
    ...
    pm.handleUpdateData(['references'], updatedRefs);

    // Re-verify async if URL changed
    if (updates.url !== undefined && oldRef && updates.url !== oldRef.url) {
      const changedRef = updatedRefs.find((r: any) => r.id === id);
      if (changedRef) {
        try {
          const { verifyReferencesBatch } = await import('./services/referenceVerificationService.ts');
          verifyReferencesBatch([changedRef]).catch(() => {});
        } catch (_e) { /* non-critical */ }
      }
    }
  };
      ...
      pm.setProjectData((prev: any) => {
        ...
        for (const repaired of repairedRefs) {
          const idx = refs.findIndex(
            (r: any) => r.id === repaired.id  // ★ EO-159 BUG 17: Match by unique id, not mutable fields
          );
          if (idx !== -1) {
...
// ── /EO-148b ────────────────────────────────────────────────────
  ...
  const handleCollectAllReferences = async () => {
    ...
    try {
      ...
      const currentRefs = Array.isArray(pm.projectData.references) ? pm.projectData.references : [];
      // ★ EO-159 BUG 21: Better dedup using doi > url > title+year
      const deduped = newRefs.filter((nr: any) => {
        return !currentRefs.some((cr: any) => {
          // Match by DOI first (most reliable)
          if (cr.doi && nr.doi && cr.doi.trim() === nr.doi.trim()) return true;
          // Match by normalized URL
          if (cr.url && nr.url && _appNormalizeUrl(cr.url) === _appNormalizeUrl(nr.url)) return true;
          // Match by title + year (case-insensitive)
          if (cr.title && nr.title && cr.year && nr.year &&
              cr.title.trim().toLowerCase() === nr.title.trim().toLowerCase() &&
              String(cr.year) === String(nr.year)) return true;
          return false;
        });
      });
      if (deduped.length === 0) {
  ...
  >
    ✕ {language === 'si' ? 'Prekliči generiranje' : 'Cancel generation'}
      ...
      <AddReferenceModal
        ...
        sectionLabels={referenceSectionLabels}
        currentReferences={Array.isArray(pm.projectData.references) ? pm.projectData.references : []}
      />
